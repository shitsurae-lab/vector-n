...
export const MorphingContainer = ({
  ...
  return (
    <div
      ref={containerRef}
      className={`flex flex-col ${isEven ? 'md:flex-row-reverse' : 'md:flex-row'} items-center gap-12 md:gap-24 py-24 px-6 md:px-16`}
    >
      {/* 左側：モーフィング画像領域 */}
      ...
      <div
        ref={contentRef}
        className='basis-full md:basis-2/5 space-y-8 text-left'
      >
        <div className='text-6xl font-light opacity-10 leading-none tabular-nums'>
          {num}
        </div>
        <div className='space-y-3'>
          ...
          <p className='text-lg tracking-[0.3em] font-medium text-gray-700'>
            {jaTitle}
          </p>
        </div>
        <div className='py-6 border-y border-black/10 text-sm leading-relaxed tracking-wider text-gray-500 whitespace-pre-wrap'>
          {category}
        </div>
        <div className='pt-4'>
          <Link
            href={link}
            className='inline-flex items-center gap-6 px-12 py-4 border border-black rounded-full hover:bg-black hover:text-white transition-all duration-500 group uppercase text-[10px] font-bold tracking-[0.25em]'
          >
            {ctaText}
