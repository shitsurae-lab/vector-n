...
これを受け、**主要なアニメーションをFramer Motionへ移行**することを決断。`AnimatePresence`・`motion` コンポーネント・`variants`による宣言的なアニメーション設計に刷新し、Reactのライフサイクルに沿った保守性の高い実装を実現しました。なお、一部のスクロール連動演出にはGSAP / ScrollTriggerを併用しています。
...
### 8. 背景透過WebPとFramer Motionによる「ロケット射出」スクロールボタン

標準的なLottie実装で発生しがちなレンダリングバグ（背景の透過エラーや点滅）を回避するため、After Effectsから「RGB+アルファ」で書き出した高品質な**アニメーションWebP**を採用しました。

- **シームレスな状態遷移**: `useState` と `useRef` を組み合わせ、待機時（ループ）とクリック時（単発発射）のWebP素材を瞬時に切り替え。
- **Headerとの完全同期**: Headerの非表示ロジック（100pxスクロールかつ下方向）と判定を同期させ、Headerが隠れると同時に右下からロケットが出現するバグのないユーザー体験を提供。
- **スクロール干渉のガード**: 発射アニメーション実行中にスクロール値が急変しても、`isLaunching`フラグで判定をシャットアウトすることで、アニメーションが途中でキャンセルされる現象を完全に防止。
- **ダイナミックなExit演出**: Framer Motionの `AnimatePresence` を活用。WebP内でのキャラの上昇動きに合わせ、コンポーネント自体を `y: -120vh` へ突き抜けさせることで、画面外へ射出されるロケットのような演出を実現
...
| **Michroma**      | カテゴリーラベル / ナビゲーション | テック・未来的エッセンスの注入。幾何学的な精密さを表現          |
...
| **アセット選定の最適化**             | トップへ戻るボタンについてLottieの描画バグ（透過・点滅）を解消するため、背景透過WebPアニメーションへ転換。AEから「RGB+アルファ」で書き出し。 |
| **トップへ戻るボタン(ロケット)演出** | Framer Motion と useRef を駆使し、Header非表示と同期した出現ロジック、およびクリック時の画面外（y: -120vh）への「突き抜け」演出を実装。      |
