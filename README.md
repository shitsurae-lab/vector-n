...
| **Michroma** | ナビゲーション / セクションタイトル / 本文 | 機械的な精密さと統一感。システムフォントをフォールバックに置くことで可読性を担保 |

Michromaをベースフォントとして全体に適用し、字間（letter-spacing）を **0.15em 単位で手動調整**。表現しきれない文字はシステムフォントでシームレスに補完しながら、一貫したリズムを作り出しています。
